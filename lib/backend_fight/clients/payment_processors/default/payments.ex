defmodule BackendFight.Clients.PaymentProcessors.Default.Payments do
  @moduledoc """
  Client module for interacting with the Default Payment Processor API.

  This module provides functions to:
  - Check the health status of the payment processor service.
  - Send a payment request using validated parameters.

  All requests use a base URL configured via the `:default_payment_processor` key in `:backend_fight` config.
  """
  ...
  @doc """
  Sends a payment creation request to the default payment processor.
  ...
  def create(params) when is_map(params) do
    with %Changeset{valid?: true} = changeset <- PaymentParams.changeset(params),
      ...
      case HTTP.post(url, json, default_headers(), default_options()) do
  ...
  defp base_url, do: Application.get_env(:backend_fight, :default_payment_processor)[:base_url]
end
